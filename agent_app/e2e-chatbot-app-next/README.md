...
## Banking Workflow Customizations

This fork extends the upstream Databricks chatbot template with domain-specific branding, a workflow-aware sidebar with filtering and progress badges, and end-to-end persistence of agent workflow state from the SSE stream through the database to the UI. The reference implementation is a private-banking assistant, but every piece below is generic to any deterministic multi-stage agent with async human-in-the-loop steps.

### Workflow state flow

The agent server emits workflow metadata via `custom_outputs` in its SSE stream. The frontend captures, persists, and displays this metadata:

```mermaid
flowchart LR
    Agent["Agent Server<br/>(custom_outputs in SSE)"] --> Provider["providers-server.ts<br/>(workflowMetadataStore)"]
    Provider --> ChatRoute["chat.ts<br/>(getAndClearWorkflowMetadata)"]
    ChatRoute --> DB["Database<br/>(updateChatWorkflowState)"]
    DB --> Sidebar["Sidebar<br/>(badges + filters)"]
```

**Key files:**

- `packages/ai-sdk-providers/src/providers-server.ts` — `WorkflowMetadata` interface, in-memory `workflowMetadataStore`, and `getAndClearWorkflowMetadata()`. The custom `databricksFetch` intercepts SSE responses and stores `custom_outputs` keyed by conversation ID.
- `server/src/routes/chat.ts` — after the stream completes, calls `getAndClearWorkflowMetadata(id)` and persists `workflow_stage`, `workflow_intent`, and `workflow_customer_name` to the database via `updateChatWorkflowState`.

### Database schema additions

The `chat` table (`packages/db/src/schema.ts`) has three columns beyond the upstream template:
...
Two components are customized to surface workflow state:
...
`GET /api/history` (`server/src/routes/history.ts`) accepts three additional query parameters:
...
`POST /api/internal/background-check-received` (`server/src/routes/internal.ts`) is called by the agent server or the `send_background_check.py` CLI after injecting a background-check result into the LangGraph checkpoint. It updates the chat's `stage` to `BACKGROUND_CHECK_RECEIVED` in the database so the sidebar badge reflects the new state before the user sends their next message.

- **Body**: `{ "chatId": "<thread_id>" }`
- **Auth**: none (intended for localhost / internal use only)
- **Response**: `200 { "success": true }` on success

### Branding

The following files contain the banking-assistant branding and copy — swap these first when retargeting to another domain:
